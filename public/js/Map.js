...
export default class Map {
     ...
     */
    constructor(width, height, cellSize, roomSize, images) {
        this.width = width;
        this.height = height;
     ...
     */
    update() {
        if (this.roomCount >= this.roomSize)
            return;
        this.roomCount++;
        let freePositions = [];
        if (this.value.length > 0) {
            ...
            // 4. Generate a cell
            let cell = this.generateCell(randomRoomX + 1 * randomDirection[0], randomRoomY + 1 * randomDirection[1]);
